...
export default () => {
  ...
  api.post('/generateRentChallenge', json(), (req, resp) => {
    ...
    const namePairBuffer =
      new Serialize.SerialBuffer({ textEncoder: new util.TextEncoder(), textDecoder: new util.TextDecoder() })
    ...
    const kPrivElliptic = PrivateKey.fromString(privateKeyWif).toElliptic(ec)
  ...
  api.post('/enroll', json(), (req, resp) => {
    const { accountName, hostname, webauthnPublicKey } = req.body
    users[accountName] = decodeWebauthnPublicKey(webauthnPublicKey, hostname)
    resp.json({ status: 'ok' })
  })
