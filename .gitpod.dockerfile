...
RUN yes | apt-get update && apt-get install -y wget sudo curl
...
RUN wget https://github.com/EOSIO/eosio.contracts/archive/v1.6.0.tar.gz
RUN mkdir -p /eosio.contracts
RUN tar xvzf ./v1.6.0.tar.gz -C /eosio.contracts
RUN mv /eosio.contracts/eosio.contracts-1.6.0 /opt/eosio/bin/contracts
RUN mv /opt/eosio/bin/contracts/eosio.contracts-1.6.0 /opt/eosio/bin/contracts/eosio.contracts
